...
public class UndirectedValuedGraph extends UndirectedGraph<UndirectedNode> {
	...
	public int[][] toAdjacencyMatrixCosts() {
		...
		for (UndirectedNode n : nodes) {
			for (UndirectedNode sn : n.getNeighbours().keySet()) {
				matrix[n.getLabel()][sn.getLabel()] = n.getNeighbours().get(sn);
			}
		}
		return matrix;
	}
	...
	public static void main(String[] args) {
		...
		System.out.println("\n************\nTests\n");
		System.out.println("isEdge() [0][3] (expected : false) - result : "
				+ al.isEdge(new UndirectedNode(0), new UndirectedNode(3)));
		System.out.println("isEdge() [0][2] (expected : true) - result : "
				+ al.isEdge(new UndirectedNode(0), new UndirectedNode(2)));
